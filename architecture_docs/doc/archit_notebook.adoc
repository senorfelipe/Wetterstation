...
** Die Datenakquise und anschließende Übertragung an den DB-Server erfolgt auf einem Raspberry Pi 4. Zwar hat dieser mittlerweile genügend Leistung um diesen Punkt zu vernachlässigen, jedoch sollte der externe Akku nicht unnötig beansprucht werden. 
* Robustheit gegen mögliche Fehler
** Das System soll nach Fertigstellung auf dem Flugplatz des MFCR im Freien installiert werden, was den Wartungsvorgang deutlich erschwert. Fehlerfälle müssen demnach sorgfältig behandelt werden.
...
** um dies zu unterstützen und die Entwicklung zu vereinfachen auf Clean Code und Dokumentation wertgelegt werden 
...
** 
. Kommunikation zwischen Raspi und Webserver bzw. DB-Server über eine Rest-Schnittstellen über HTTPS 
